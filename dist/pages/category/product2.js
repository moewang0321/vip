require(['../../scripts/conf/config'] , function(){
    require(['jquery' , 'smoothP' , 'jqCookie'], function($){
        ...
        */
        ...
        $.ajax({
            url:'https://mapi.vip.com/vips-mobile/rest/shopping/pc/product/detail/v5?callback=detailInfoCB&productId='+ proId +'&warehouse=VIP_BJ&client_type=pc&fdc_area_id=911101114112&brandid=1710631773&api_key=70f71280d5d547b2a7bb370a529aeea1&app_name=shop_pc&app_version=4&device=1&mars_cid=1568805197047_731e0f18607023a4fb2184110bc23ac9&source_app=pc&mobile_platform=1',
            type:'POST',
            contentType: 'application/x-www-form-urlencoded;charset=utf-8',
            data: {},
            dataType: "JSONP",
            ...
            success: function(data){
                ...
                /* 标题填充 */
                $('title').html(`${itemData.brandStoreName + itemData.title}`)
                ...
                /* 商品参数 */
                var attrSpecProps = itemData.attrSpecProps;
                var length = attrSpecProps.length;
                var arrSingle = new Array;
                var arrOdd = new Array;
                var tBody = document.createElement('tbody');
                tBody.innerHTML =   ``;
                $('.dc-table').append(tBody);
                for (let m = 0; m < length / 2; m++) {
                    for (let j = 0; j < 2; j++) {
                        var tr = document.createElement('tr');
                        tr.innerHTML = ``;
                        $('.dc-table tbody').append(tr);
                    }
                }
                for (let i = 0; i < attrSpecProps.length; i++) {
                    let str = '';
                    for (let j = 0; j < attrSpecProps[i]['values'].length; j++) {
                        str = str + attrSpecProps[i]['values'][j]['optionName'] + ' ';
                    }
                    attrSpecProps[i]['values'] = str;
                    
                }
                for (let i = 0; i < length; i++) {
                    if(i % 2 == 0){
                        arrSingle.push(attrSpecProps[i]);
                    }else{
                        
                        arrOdd.push(attrSpecProps[i]);
                    }
                ...
                for (let i = 0; i < length / 2; i++) {
                    ...
                    th1.innerHTML = `${arrSingle[i]['attributeName']}:`;

                    var td1 = document.createElement('td');
                    td1.innerHTML = `${arrSingle[i]['values']}`;
                    var th2 = document.createElement('th');
                    setAttributes(th2 , {
                        'class' : 'dc-table-tit'
                    })
                    th2.innerHTML = `${arrOdd[i]['attributeName']}:`;

                    var td2 = document.createElement('td');
                    td2.innerHTML = `${arrOdd[i]['values']}`;
                    $('.dc-table tbody tr').eq(i).append(th1);
                    $('.dc-table tbody tr').eq(i).append(td1);
                    $('.dc-table tbody tr').eq(i).append(th2);
                    $('.dc-table tbody tr').eq(i).append(td2);
        ...
        $.ajax({
            url:'https://stock.vip.com/detail/?callback=stock_detail&merchandiseId='+ proId +'&is_old=0&areaId=911101114112&_=1569197255342',
            type:'POST',
            contentType: 'application/x-www-form-urlencoded;charset=utf-8',
            data: {},
            dataType: "JSONP",
        ...
        */
        ...
        $.ajax({
            url:'https://pcapi.vip.com/vre/index.php?callback=getRecommendProductsCB&act=get_recommend_merchandise_list&product_id='+ proId +'&count=50&abtest_id=132000&warehouse=VIP_BJ&_=1568898953541',
            type:'POST',
            contentType: 'application/x-www-form-urlencoded;charset=utf-8',
            data: {},
            dataType: "JSONP",
